...
# tile_num = 8    # classes = 100
# tile_num = 5    # classes = 10
tile_num = 4    # classes = 4

if __name__ == "__main__":
    outname = f'../user_data/permutations_{tile_num}'
    np.set_printoptions(threshold=np.inf)
    P_hat = np.array(list(itertools.permutations(list(range(tile_num)), tile_num)))
    n = P_hat.shape[0]
